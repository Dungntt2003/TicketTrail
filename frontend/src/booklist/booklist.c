...
static gboolean on_button_press(GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
    if (event->type == GDK_BUTTON_PRESS) {
        ...
        // Loop through tickets to determine which button was pressed
        for (size_t i = 0; i < ticket_count; ++i) {
            ...
            double print_button_y = 111 + i * (ticket_height + 30) + ticket_height + 20; // Adjust for ticket spacing

            // Check if click is within the button's bounds for this ticket
            if (event->x >= print_button_x && event->x <= print_button_x + button_width &&
                ...
                // Run the dialog and handle user's choice
                if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
                    char *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));

                    // Generate and save the PDF for the selected ticket
