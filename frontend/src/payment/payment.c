...
GtkWidget* create_payment_window() {
    GtkWidget *payment_window, *drawing_area;

    payment_window = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    // gtk_window_set_title(GTK_WINDOW(payment_window), "Payment");
    // gtk_window_fullscreen(GTK_WINDOW(payment_window));
    // gtk_window_set_resizable(GTK_WINDOW(payment_window), FALSE);

    drawing_area = gtk_drawing_area_new();
    g_signal_connect(drawing_area, "draw", G_CALLBACK(on_payment_draw), NULL);
    gtk_container_add(GTK_CONTAINER(payment_window), drawing_area);
